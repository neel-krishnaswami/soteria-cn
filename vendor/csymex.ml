...
module SYMEX = Soteria.Symex.Make (Bv_solver.Z3_solver (Typed))
...
module Freeable = Freeable.Make (CSYMEX)
module Pmap_sig = Sym_pmap.S (CSYMEX)

module Pmap_direct_access =
  Sym_pmap.Direct_access_patricia_tree (CSYMEX)

module Lazy_map = Sym_pmap.Lazy (CSYMEX)
module Tree_block = Tree_block.Make (CSYMEX)
module Concrete_map = Sym_pmap.Concrete (CSYMEX)
