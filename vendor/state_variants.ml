...
module Heap_direct_access = Pmap_direct_access (S_loc) (Block)
module Heap_lazy = Lazy_map (S_loc) (Block)

module type Heap_map_sig =
  Pmap_sig(S_loc).S with type codom := Block.t and type codom_syn := Block.syn

module Make (Heap_map : Heap_map_sig) = struct
  module Heap = struct
    include Heap_map
...
let get_global (sym : Cerb_frontend.Symbol.sym) =
  ...
  let produce_tree_block
    ...
    let t =
      match t with None -> { heap = None; globs = None } | Some t -> t
    in
    ...
    | Struct _ ->
        ...
        let rec aux members_ofs members values_acc =
          ...
          | (Layout.Padding size, ofs) :: rest_ofs, members ->
              let** () =
                consume_any' loc (BV.usizei ofs +!!@ offset) (BV.usizeinz size)
              in
...
module Direct_access = Make (Heap_direct_access)
module Lazy = Make (Heap_lazy)
